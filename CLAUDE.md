...
- `pdf_url`: 동향형에도 **선택적으로** 쓸 수 있다. 상세 템플릿이 유형과 무관하게 PDF 버튼을
  렌더하므로, 원문에 안정적인 GET 직링크가 있으면(예: OECD `content/dam/...`) 채운다.
  동향형의 기본 필드는 institution/title/region/date/source_url/abstract_source 6개이고,
  `pdf_url` 은 여기에 더할 수 있는 유일한 선택 필드다.
...
- 구분(kind)에 `기본연구`(RR), `수탁연구`(CRR/CRM), `이슈페이`(IP/CIP), `연구자료`(RM),
  `현안보고` 등이 섞여 있다. 포럼 자료집·기획기사·사업 결과보고서·국제회의 자료
  (RM/CRM 다수)는 연구가 아니므로 제외한다.
- **실제로 요약할 수 있는 것은 RR·IP 뿐이다.** (2026-09 확인)
  CRR·CIP 는 수탁연구라 상세에 "저작권이 본원에 있지 않아 원문 파일을 제공할 수
  없습니다" 안내만 뜨고 PDF 가 없다(`downloadAction(...)` 호출 자체가 없다). 사이트엔
  초록도 없으므로 요약 근거를 얻을 방법이 없다. 표본 확인한 CRR·CIP 13건이 모두
  그러했으니, **상세를 열었을 때 `downloadAction('...')` 인자가 없으면 즉시 건너뛴다.**
- **1990년대 ~ 2010년대 초 자료는 분석 대상에서 제외한다.** (사용자 지시, 2026-09)
  관심 키워드로는 걸리지만(교육용 소프트웨어 심의기준, 데이터 기반 교육정책 분석
  시리즈 등) 현재 정책·기술 맥락과 거리가 멀어 큐레이션 가치가 낮고, 스캔본이라
  OCR 비용만 크다. CSV 에 `관련=o, 요약=X` 로 남아 있어도 **요약하지 않는다.**
...
- **컬럼**: 기관코드, 기관명, 식별자, 제목, 발행, source_url, 관련여부(o/X),
  ...
  - `요약여부`: `src/reports/<code>/` 의 기존 식별자와 대조해 자동 표시.
    edpolicy-intl(10)만은 상세페이지가 없어 **외부 원문 URL(정규화)** 로 대조한다.
    (게시판이 과거엔 '월별 주요동향' 내부 상세글, 최근엔 개별 기사 외부 직링크형이라
    크롤러는 외부 직링크 항목만 담는다.)
  - **URL 정규화**(`norm_url`): 같은 문서가 다른 URL 로 잡혀 대조가 실패하지 않도록
    `utm_*`, 목록 문맥 파라미터(`posInSet`·`queryId`·`hub`), 그리고 unesdoc 의
    경로 끝 `.locale=xx` 를 제거한다. 게시판이 같은 원문을 여러 링크 형태로 올려도
    한 건으로 합쳐진다. (2026-09 도입)
...
6. **알려진 예외/부채**: 예) 스캔 PDF 라 OCR 필요/건너뛴 건, 수탁연구여서 원문 PDF 가
   제공되지 않아 요약하지 못한 건 등.
   - **KEDI 수탁연구(CRR·CIP)**: 원문 PDF 미제공이라 요약 불가. RR·IP 만 가능하다.
     ([[KEDI 사이트 활용법]] 참고)
   - **KEDI 1990년대~2010년대 초 자료**: 사용자 지시로 분석 대상에서 제외. CSV 에
     `관련=o, 요약=X` 로 계속 남지만 잔여 건수로 세지 않는다.
   - *해소됨(2026-09)*: edpolicy-intl 이 정식 문서 URL 로 저장돼 게시판 링크와 달라
     요약여부 대조가 실패하던 문제는 `norm_url` 정규화로 해결했다.
